...
class async
{
    ...
    function curlAsync($data,$url)
    {
        ...
        try{
            ...
            if($result['response']){
                $data = ['code'=>1,'msg'=>'发送成功','data'=>$result];
            }else{
                $data = ['code'=>0,'msg'=>'发送失败','data'=>$result];
            }
        }catch(Exception $e){
//            var_dump(curl_error($ch));//显示错误原因
            $data = ['code'=>0,'msg'=>'发送失败','data'=>curl_error($ch)];
        }
        curl_close($ch);
        return $data;
    }
     ...
     * @return array
     */
    function sockGet($host, $url, $param)
    {
